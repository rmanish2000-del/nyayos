...
| m-1 direct `status` update on disputes | **Open** — `0001` still grants `update (title, status, category_label)` to `nyayos_authenticated`; A-041 could not change SQL | §2, §8 item 6 |
